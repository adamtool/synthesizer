 ...
 *
 * Currently it also has the type-2 trap separately calculated but already
 * stores the transition leading into the current state.
 *
 * So it's currently the same as DistrSysBDDASafetySolver but includes the last
 * transition and uses the local nondeterminism version.
 *
 *
 * @author Manuel Gieseking
 */
public class DistrSysBDDASafetyLocNDetSolver extends DistrSysBDDSolver<Safety> implements DistrSysBDDType2Solver {

    // Domains for predecessor and successor for each token
    private BDDDomain[][] TYPE;
    private BDDDomain[] LAST_T;
     ...
     */
    DistrSysBDDASafetyLocNDetSolver(DistrSysBDDSolvingObject<Safety> obj, BDDSolverOptions opts) throws NotSupportedGameException, NetNotSafeException, NoSuitableDistributionFoundException, InvalidPartitionException {
        super(obj, opts);
    }
     ...
     * last_t| and the same for the successor
     */
    @Override
    protected void createVariables() {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            LAST_T[i] = getFactory().extDomain(getSolvingObject().getGame().getTransitions().size() + 1);
        }
...
// %%%%%%%%%%%%%%%%%%%%%%%%%%% END INIT %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    protected String decodeDCS(byte[] dcs, int pos) {
        ...
        sb.append(BDDTools.getTransitionByBin(getSolvingObject().getGame(), dcs, LAST_T[pos]));
        return sb.toString();
    }
...
//                        continue;
        ...
        // last transition
        sys2.andWith(LAST_T[1].ithVar(getSolvingObject().getGame().getID(t)));
...
//    }
    ...
    protected BDD calcWinningDCSs(Map<Integer, BDD> distance) throws CalculationInterruptedException {
        ...
        BDD fixedPoint = attractor(getBadDCSs(), true, distance, false, null).not().and(getBufferedDCSs());//fixpointOuter();
...
// %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Some helping calculations %%%%%%%%%%%%%%%%
    ...
    protected BDD getVariables(int pos) {
        ...
        variables.andWith(LAST_T[pos].set());
        return variables;
    }
