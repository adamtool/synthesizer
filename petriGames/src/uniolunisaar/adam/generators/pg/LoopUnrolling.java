...
public class LoopUnrolling {
     ...
     * @return
     */
    public static PetriGame createESafetyVersion(int nb_unrolling, boolean withNewChain, boolean withPartitioning) {
        if (nb_unrolling < 1) {
            throw new RuntimeException("less than 1 unrolling does not show the interesting situation.");
        }

        PetriGame net = PGTools.createPetriGame("ESafety version of loop unrolling example (" + ((withNewChain) ? "with" : "without") + " new chains)  with " + nb_unrolling + " unrollings.");
