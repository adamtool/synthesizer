...
public class Escape {
     ...
     */
    public static PetriGameWithTransits createESafetyVersion(int nb_sys, int nb_env, boolean withPartitioning) throws FileNotFoundException, ModuleException {
        if (nb_sys < 0 || nb_env < 0) {
            throw new RuntimeException("No system and no environment is not an interesting example.");
        }
        PetriGameWithTransits net = createGeneralNet(nb_sys, nb_env, withPartitioning);
        addESafetyWinCondition(net, nb_sys, nb_env, withPartitioning);
        return net;
    }

    private static PetriGameWithTransits createGeneralNet(int nb_sys, int nb_env, boolean withPartitioning) {
        PetriGameWithTransits net = PGTools.createPetriGame("Escape with " + nb_sys + " escapees against " + nb_env + " captors");
    ...
    private static void createEnvChoice(PetriGameWithTransits net, int i, Place start) {
    ...
    private static Place createSysChoice(PetriGameWithTransits net, int i) {
    ...
    private static Place addBadAndGood(PetriGameWithTransits net, int i) {
